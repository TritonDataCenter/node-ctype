...
function testUint8()
{
  ...
  result = parser.readStruct([ { item: { type: 'uint8_t' } } ], buffer, 0);
...
function testUint16()
{
  ...
  result = parser.readStruct([ { item: { type: 'uint16_t' } } ], buffer, 0);
...
function testSint16()
{
  ...
  parser = new mod_ctype.Parser({ endian: 'little' });
  buffer = new Buffer('8000', 'hex');
  result = parser.readStruct([ { item: { type: 'int16_t' } } ], buffer, 0);
  ASSERT.equal(result['size'], 2);
}
...
function testUint32()
{
  ...
  result = parser.readStruct([ { item: { type: 'uint32_t' } } ], buffer, 0);
...
function testSint32()
{
  ...
  parser = new mod_ctype.Parser({ endian: 'little' });
  buffer = new Buffer('80000000', 'hex');
  result = parser.readStruct([ { item: { type: 'int32_t' } } ], buffer, 0);
  ASSERT.equal(result['size'], 4);
}
...
function testUint64()
{
  ...
  result = parser.readStruct([ { item: { type: 'uint64_t' } } ], buffer, 0);
...
function testSint64()
{
  ...
  parser = new mod_ctype.Parser({ endian: 'little' });
  buffer = new Buffer('8000000000000000', 'hex');
  result = parser.readStruct([ { item: { type: 'int64_t' } } ], buffer, 0);
  ASSERT.equal(result['size'], 8);
}
